 ...
 * between requests.
 */
const headers = CONFIG.jupiterApiKey && CONFIG.jupiterQuoteApi.includes('api.jup.ag')
 ...
 */
export function fetchBuyQuote(mint, solAmountUi) {
    return fetchQuote({
 ...
 */
export function fetchSellQuote(mint, tokenAmountUi) {
    return fetchQuote({
        inputMint: mint,
...
async function fetchQuote(opts) {
    ...
    if (opts.dexes)
        params.set('dexes', opts.dexes);
    try {
        const q = await fetchQuoteJson(params);
        const outAmount = Number(q.outAmount);
        if (!isFinite(outAmount) || outAmount <= 0)
            throw new Error('bad outAmount');
 ...
 */
async function fetchQuoteJson(params) {
    ...
    for (;;) {
        ...
        if (r.status !== 429)
            throw new Error(`HTTP ${r.status} from ${CONFIG.jupiterQuoteApi}: ${body}`);
