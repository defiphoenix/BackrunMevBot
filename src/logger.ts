 ...
 */
function prettySegments(msg: string, accent: (s: string) => string): string {
  ...
  const kv = segments.map((seg) => {
    ...
    if (eq > 0 && !seg.slice(0, eq).includes(' ')) {
      return [seg.slice(0, eq), seg.slice(eq + 1)] as const;
    }
    return [null, seg] as const;
  });
...
export const log = {
  ...
  opportunity(msg: string) {
    console.log(
      `${dim(`[${ts()}]`)} ${greenBold('★ OPPORTUNITY')}${green(prettySegments(msg, bold))}`,
    );
    if (CONFIG.logFile) {
      try {
