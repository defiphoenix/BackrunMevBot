...
import type { OpportunityServer } from './server.js';
import type { ArbOpportunity, PumpSwapTradeEvent } from './types.js';
...
export class ArbDetector {
  constructor(
    private meteora: MeteoraPoolCache,
    private server?: OpportunityServer,
  ) {}

  async onTrade(e: PumpSwapTradeEvent): Promise<void> {
    if (!(e.quoteAmount > CONFIG.bigSellThresholdSol)) return;
    ...
    const summary =
      `detector=single-hop backrun (PumpSwap -> Meteora) | ` +
