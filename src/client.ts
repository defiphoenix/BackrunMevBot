...
const sol = (n: number, digits = 4) => `${n >= 0 ? '+' : ''}${n.toFixed(digits)} SOL`;
const price = (n: number) => `${n.toExponential(6)} SOL/token`;
const short = (addr: string) => (addr.length > 12 ? `${addr.slice(0, 4)}…${addr.slice(-4)}` : addr);

function printOpportunity(o: ArbOpportunity, tag = '★ OPPORTUNITY'): void {
...
function connect(): void {
  ...
  ws.on('message', (raw) => {
    ...
    if (frame.type === 'hello') {
      const history: ArbOpportunity[] = frame.data?.history ?? [];
      ...
      for (const o of [...history].reverse()) printOpportunity(o, '★ RECENT');
    } else if (frame.type === 'arb_opportunity') {
      printOpportunity(frame.data as ArbOpportunity);
    }
  });
