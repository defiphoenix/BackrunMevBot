...
export class OpportunityServer {
  ...
  private wss = new WebSocketServer({ server: this.http, path: '/ws' });
  private history: ArbOpportunity[] = [];
  private published = 0;
  ...
  start(): void {
    this.http.listen(CONFIG.apiPort, () => {
      log.info(
        `Opportunity API listening on http://localhost:${CONFIG.apiPort} ` +
  ...
  publish(opp: ArbOpportunity): void {
    this.published++;
    this.history.unshift(opp);
    if (this.history.length > CONFIG.opportunityHistorySize) this.history.pop();
